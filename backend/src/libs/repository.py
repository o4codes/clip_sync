...
class BaseRepository:
    ...
    async def nested_list(
        ...
        Args:
            ...
            skip (Optional[int]): The number of nested documents to skip
            filter_ (Dict[str, Any]): A filter statement to filter result
        Returns:
    ...
    async def nested_count(
        ...
        Args:
            ...
            filter_ (Dict[str, Any]: dict expression of filter to apply for results
    ...
    async def nested_create(
        ...
        main_doc_id: ObjectId,
        field: str,
        data: Union[Dict[str, Any], str, int, ObjectId],
        nested_model_class: Optional[Type[DbModel]] = None,
    ) -> Optional[Union[DbModel, dict, str, int, ObjectId]]:
        """
    ...
    async def nested_remove(
        self,
        main_doc_id: ObjectId,
        nested_doc_id: Union[ObjectId, Any],
        field: str,
        filter_: Optional[Dict[str, Any]] = None,
    ) -> bool:
        """
        ...
        Args:
            ...
            filter_ (Optional[Dict[str, Any]]): Optional filter to search for documents
            field (str): The name of the field containing the nested document
            nested_doc_id (Union[ObjectId, Any]): The ID or value of the nested document to remove
        ...
        filter_ = {
            "_id": main_doc_id,
            **filter_,
            f"{field}": {"$elemMatch": {"$eq": nested_doc_id}},
        }

        options = {
            "return_document": ReturnDocument.AFTER,
            "projection": {field: {"$elemMatch": {"$eq": nested_doc_id}}},
        }

        removed_doc = await self.collection.find_one_and_update(
            filter_, {"$pull": {field: {"$in": [nested_doc_id]}}}, **options
        )
